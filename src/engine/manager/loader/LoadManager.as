package engine.manager.loader
{
	...
	import engine.manager.version.VersionManager;
	...
	public class LoadManager extends EventDispatcher
	{
		...
		public function setup(fa:IFacade):void
		{
			_facade = fa;
		}
		...
		{
			var evt:LoadEvent = new LoadEvent(LoadEvent.LOAD_START);
		...
		private function _loadPlugin():void
		{
			...
			}else//如果没有缓存 就加载新的
			{
				...
				_loader.load(new URLRequest(_currentItem.url+VersionManager.ver),_context); 
			}
		}
		...
		private function _showLoadTip(per:Number):void
		{
			...
			evt.msg = "["+ _loadCount+"/"+_total+"]"+" "+(Number(per*100).toFixed(1))+"% "+ _currentItem.msg;
